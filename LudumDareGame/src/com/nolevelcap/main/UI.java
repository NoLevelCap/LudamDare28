...
public class UI {
	...
	public UI(SpriteBatch draw, Player player, MouseHandling mouse, BitmapFont font){
		...
		try {
			...
			this.iconGold = new Texture(draw.getResource("Player/coin.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		initHealth();
	...
	public void render(){
		...
		draw.draw(health[player.health], 30, 625, 512, 64);
		
		draw.draw(buttonBack, 800, 625, 128, 64);
		draw.draw(iconInt, 820, 640, 32, 32);
		draw.setColor(Color.BLACK);
		font.drawBigText(draw, player.intellect+"", 855, 650, 0.5f);
		draw.setColor(Color.WHITE);
		
		draw.draw(buttonBack, 1000, 625, 128, 64);
		draw.draw(iconGold, 1020, 640, 32, 32);
		draw.setColor(Color.BLACK);
		font.drawBigText(draw, player.gold+"", 1055, 650, 0.5f);
		draw.setColor(Color.WHITE);
